...
namespace Spa.KlarnaCheckout.Web.Managers
{
    public class SpaKlarnaCheckoutPaymentMethod : PaymentMethod
    {
        ...
        private const string _klarnaModeStoreSetting = "Spa.KlarnaCheckout.Web.Mode";
        private const string _klarnaAppKeyStoreSetting = "Spa.KlarnaCheckout.Web.AppKey";
        private const string _klarnaAppSecretStoreSetting = "Spa.KlarnaCheckout.Web.SecretKey";
        private const string _klarnaTermsUrl = "Spa.KlarnaCheckout.Web.TermsUrl";
        private const string _klarnaCheckoutUrl = "Spa.KlarnaCheckout.Web.CheckoutUrl";
        private const string _klarnaConfirmationUrl = "Spa.KlarnaCheckout.Web.ConfirmationUrl";
        private const string _klarnaPaymentActionType = "Spa.KlarnaCheckout.Web.PaymentActionType";

        private const string _klarnaSalePaymentActionType = "Sale";

        public SpaKlarnaCheckoutPaymentMethod()
			: base("SpaKlarnaCheckout")
		{
        }
        ...
        public override CaptureProcessPaymentResult CaptureProcessPayment(CaptureProcessPaymentEvaluationContext context)
        {
            ...
            var connector = Connector.Create(AppSecret, Connector.TestBaseUri);
        ...
        private ProcessPaymentResult ProcessKlarnaOrder(KlarnaLocalization localization, ProcessPaymentEvaluationContext context)
        {
            ...
            order =
                new Order(connector)
                {                    
                    Location = new Uri(GetCheckoutBaseUrl(localization.Currency)),
                    ContentType = _contentType
                };
            order.Create(data);
            order.Fetch();
            ...
            var html = "<div>" + gui["snippet"].Value<string>().Replace("{(", "{ (").Replace(")}", ") }") + "</div>";
        ...
        private PostProcessPaymentResult PostProcessKlarnaOrder(PostProcessPaymentEvaluationContext context)
        {
            ...
            retVal.OrderId = context.Order.Number; // Was Id
        ...
        private KlarnaLocalization GetLocalization(string currency, string country)
        {
            ...
            {
                return new KlarnaLocalization
                {
                    CountryName = "NO",
                    Currency = "NOK",
                    Locale = "nb-no",
                    FullCountryName = "Norway"
                };
